...
def create_default_superusers(db: Session):
    try:
        # Define default superusers only
        default_superusers = [
            {
                "email": "asheerali1997@gmail.com",
                "password": "admin",
        ...
        for user_data in default_superusers:
            ...
            user = User(
                ...
                phone_number=user_data.get("phone_number"),
                role=user_data["role"],
                company_id=None  # Superusers don't need company association
            )
            db.add(user)
            print(f"Superuser '{user.email}' created.")

        db.commit()
        print("All superusers processed.")

    except Exception as e:
        db.rollback()
        print(f"Error while creating superusers: {e}")