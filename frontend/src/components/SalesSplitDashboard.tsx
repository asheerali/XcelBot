...
const SalesSplitDashboard: React.FC<SalesSplitDashboardProps> = ({ 
  ...
  if (!tableData || 
      !tableData.table1 || !Array.isArray(tableData.table1) || tableData.table1.length === 0) {
    return (
  ...
  const getNumericColumns = (table: any[]) => {
    ...
    return Object.keys(firstRow).filter(key => {
      ...
      return typeof value === 'number' || 
             (typeof value === 'string' && !value.includes('%') && !isNaN(parseFloat(value.replace(/[$,]/g, ''))));
    });
  };
  ...
  const processTableData = () => {
    ...
    const totalSalesData = tableData.table1.map((row: any) => ({
      week: `${row.Week}`,
      sales: Math.round((row['Grand Total'] || row[table1Columns.find(col => col.toLowerCase().includes('total')) || table1Columns[table1Columns.length - 1]] || 0) / 1000)
    }));

    // Calculate category percentages from table1 data - dynamic categories
    const categoryData = [];
    let totalSales = 0;
    const categories = table1Columns.filter(col => 
    ...
    tableData.table1.forEach((row: any) => {
      categories.forEach(category => {
        const value = parseFloat(String(row[category] || 0).replace(/[$,]/g, '')) || 0;
    ...
    let table2Data = [];
    if (tableData.table2 && Array.isArray(tableData.table2) && tableData.table2.length > 0) {
    ...
    if (tableData.table3 && Array.isArray(tableData.table3) && tableData.table3.length > 0) {
      ...
      const inHouseColumn = table3Columns.find(col => 
    ...
    let table4Data = [];
    if (tableData.table4 && Array.isArray(tableData.table4) && tableData.table4.length > 0) {
    ...
    const inHouseData = tableData.table1.map((row: any) => {
      const inHouseColumn = categories.find(col => 
        col.toLowerCase().includes('house') || 
        col.toLowerCase().includes('in-house')
      );
      
      const inHouseValue = parseFloat(String(row[inHouseColumn] || 0).replace(/[$,]/g, '')) || 0;
      const grandTotalColumn = table1Columns.find(col => col.toLowerCase().includes('total')) || 'Grand Total';
      const grandTotal = parseFloat(String(row[grandTotalColumn] || 0).replace(/[$,]/g, '')) || 0;
  ...
  const totalSalesValue = chartData.totalSalesData.reduce((sum, item) => sum + item.sales, 0);
  ...
  const InHouseProgressBars = () => {
    ...
    const uniquePercentages = [...new Set(chartData.inHouseData.map(d => d.percent))]
      .filter(p => p > 0)
      .sort((a, b) => a - b)
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', marginTop: '16px' }}>
        {uniquePercentages.map((percentage, index) => (
          <div key={percentage} style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ 
              width: '100%', 
              height: '24px', 
  ...
  const renderWOWBars = () => {
    ...
    const availableColumns = chartData.availableColumns.table2.length > 0 
      ? chartData.availableColumns.table2 
      : chartData.availableColumns.table4;
    
    const colors = [
      '#4D8D8D', '#2D5F5F', '#7DCBC4', '#FFCE56', '#9FE2E0', 
  ...
  return (
    <div style={{ 
      ...
      <Typography variant="caption" sx={{ color: '#666', mb: 1 }}>
        Available tables: Table1 ({chartData.availableColumns.table1.length} cols), 
        Table2 ({chartData.availableColumns.table2.length} cols), 
        Table3 ({chartData.availableColumns.table3.length} cols), 
        Table4 ({chartData.availableColumns.table4.length} cols)
      </Typography>
