...
const InHousePercentageChart: React.FC<InHousePercentageChartProps> = ({
  tableData,
}) => {
  ...
  const processDataForChart = (rawData: any[]): any[] => {
    return rawData.map((weekData) => {
      ...
      let inHousePercentage = grandTotal
        ? (inHouseValue / grandTotal) * 100
        : 0;
  ...
  const calculateTrendLine = (data: any[]): any[] => {
    ...
    if (data.length >= windowSize) {
      for (let i = 0; i < data.length; i++) {
        ...
        // Add trend line data
        result[i]["In-House_trend"] = parseFloat(
          (inHouseSum / count).toFixed(2)
        );
      }
    } else {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      const barPayload = payload.find(
        (entry: any) => entry.dataKey === "In-House"
      );
      ...
      return (
        ...
        >
          ...
          >
            In-House: $
            {barPayload.payload["In-House_raw"].toLocaleString(undefined, {
          ...
          <p style={{ margin: 0, color: "#555" }}>
            Total Sales: $
            {barPayload.payload["Grand Total_raw"].toLocaleString(undefined, {
              minimumFractionDigits: 2,
              maximumFractionDigits: 2,
            })}
          </p>
  ...
  const minPercentage =
    Math.min(...dataWithTrendLine.map((item: any) => item["In-House"])) * 0.9;
  const maxPercentage =
    Math.max(...dataWithTrendLine.map((item: any) => item["In-House"])) * 1.1;
        ...
        <Box sx={{ width: "100%", height: 300 }}>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar dataKey="In-House" fill="#8BC34A" name="In-House">
                <LabelList
                  dataKey="In-House"
                  position="top"
