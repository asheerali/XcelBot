...
const PercentageFirstThirdPartyChart: React.FC<PercentageFirstThirdPartyChartProps> = ({ tableData }) => {
  const [chartData, setChartData] = useState<any[]>([]);

  useEffect(() => {
    // Process the data for the chart whenever tableData changes
    if (tableData && tableData.table1 && tableData.table1.length > 0 && tableData.table4 && tableData.table4.length > 0) {
  ...
  const processDataForChart = (data: any): any[] => {
    ...
    return table1.map((item: any) => {
      ...
      const total = firstParty + thirdParty + parseCurrencyValue(item['Catering']) + parseCurrencyValue(item['In-House']);
      ...
      const ratioPct = total > 0 ? ((firstParty + thirdParty) / total * 100) - firstPartyPct - thirdPartyPct : 0;
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      // Get the values
      const firstPartyPct = payload.find((p: any) => p.name === "1P")?.value || 0;
      const thirdPartyPct = payload.find((p: any) => p.name === "3P")?.value || 0;
      ...
      return (
        <div style={{ 
          ...
          <p style={{ margin: 0, color: '#B9C6FF' }}>1P: {firstPartyPct.toFixed(2)}%</p>
          <p style={{ margin: 0, color: '#F3D479' }}>3P: {thirdPartyPct.toFixed(2)}%</p>
          <p style={{ 
            margin: '5px 0 0 0', 
            color: '#C687A7', 
            fontWeight: 'bold',
      ...
      <Card sx={{ mb: 4 }}>
        <CardContent>
          ...
          <Typography color="text.secondary">
            No 1P/3P data available. Please upload an Excel file with valid sales data.
          </Typography>
        </CardContent>
      </Card>
    );
