...
export function Financials() {
  ...
  const currentFinancialData = financialFiles.find(
    (f) => f.location === currentFinancialLocation
  );
  ...
  const availableYears = currentFinancialData?.data?.table1?.[0]
    ?.financials_years || ["2025", "2024"];
      ...
      const newStatsData = [
        {
          ...
          bottomLabel: "% Change",
          changeColor:
            parseFloat(weekStats.netSalesChange) >= 0 ? "#2e7d32" : "#d32f2f",
          changeDirection:
            parseFloat(weekStats.netSalesChange) >= 0 ? "up" : "down",
        },
        {
          label: "Orders",
          ...
          bottomLabel: "% Change",
          changeColor:
            parseFloat(weekStats.ordersChange) >= 0 ? "#2e7d32" : "#d32f2f",
          changeDirection:
            parseFloat(weekStats.ordersChange) >= 0 ? "up" : "down",
        },
        {
          label: "Avg Ticket",
          ...
          changeDirection:
            parseFloat(weekStats.avgTicketChange) >= 0 ? "up" : "down",
          changeColor:
            parseFloat(weekStats.avgTicketChange) >= 0 ? "#2e7d32" : "#d32f2f",
          bottomLabel: "$ Change",
        },
        {
          label: "Food Cost",
          ...
          changeDirection:
            parseFloat(weekStats.foodCostChange) >= 0 ? "up" : "down",
          changeColor:
            parseFloat(weekStats.foodCostChange) >= 0 ? "#d32f2f" : "#2e7d32",
          bottomLabel: "% Change",
        },
        {
          label: "Labor Cost",
          ...
          changeDirection:
            parseFloat(weekStats.laborCostChange) >= 0 ? "up" : "down",
          changeColor:
            parseFloat(weekStats.laborCostChange) >= 0 ? "#d32f2f" : "#2e7d32",
          bottomLabel: "% Change",
        },
        {
          label: "SPMH",
          ...
          changeDirection:
            parseFloat(weekStats.spmhChange) >= 0 ? "up" : "down",
          changeColor:
            parseFloat(weekStats.spmhChange) >= 0 ? "#2e7d32" : "#d32f2f",
          bottomLabel: "% Change",
        },
        {
          label: "LPMH",
          ...
          changeDirection:
            parseFloat(weekStats.lpmhChange) >= 0 ? "up" : "down",
          changeColor:
            parseFloat(weekStats.lpmhChange) >= 0 ? "#d32f2f" : "#2e7d32",
          bottomLabel: "% Change",
        },
      ];
    ...
    <Box sx={{ p: 2, backgroundColor: "#ffffff", minHeight: "100vh" }}>
      ...
      {financialFiles.length === 0 && (
        <Alert severity="info" sx={{ mb: 3 }}>
          No financial data available. Please upload financial files with
          dashboard type "Financials" first.
        </Alert>
      )}

      {/* Alert for implementation status */}
      {currentFinancialData?.data?.data ===
        "Financial Dashboard is not yet implemented." && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          Financial Dashboard is not yet fully implemented. Displaying available
          data.
        </Alert>
      )}
        ...
        <Grid item xs={12} md={4} lg={3}>
          <Card elevation={3} sx={{ borderRadius: 2 }}>
            <CardContent>
              <Typography
                variant="h6"
                sx={{ mb: 3, fontWeight: 500, color: "#424242" }}
              >
                Filters
              </Typography>
              ...
              <Box sx={{ mb: 3 }}>
                <Typography variant="caption" color="text.secondary">
                  Current file:{" "}
                  {currentFinancialData?.fileName || "No file selected"}
                </Typography>
                <Typography
                  variant="caption"
              ...
              <Box sx={{ mb: 3 }}>
                <Typography
                  sx={{ mb: 1, fontSize: "0.875rem", color: "#616161" }}
                >
                  Store
                </Typography>
                <FormControl fullWidth size="small">
                  <Select
                    value={
                      financialFilters.store || currentFinancialLocation || ""
                    }
                  ...
                  >
                    {financialLocations.length > 0
                      ...
                      : [
                          <MenuItem key="default1" value="0001: Midtown East">
                            0001: Midtown East
                          </MenuItem>,
                          <MenuItem key="default2" value="0002: Downtown West">
                            0002: Downtown West
                          </MenuItem>,
                        ]}
                  </Select>
