// src/pages/Financials.tsx - Updated with Redux integration and date range Redux integration
...
// Import masterFileSlice Redux actions and selectors
import {
...
// NEW: Import dateRangeSlice Redux actions and selectors for Financials Dashboard
import {
...
interface DateRangeSelectorComponentProps {
  ...
  onCancel?: () => void;
  initialDateRange?: any; // NEW: Add prop for initial date range
}
...
const DateRangeSelectorComponent: React.FC<DateRangeSelectorComponentProps> = ({
  ...
  onDateRangeSelect,
  onCancel,
  initialDateRange // NEW: Accept initial date range prop
}) => {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  
  // NEW: Initialize selectedRange from initialDateRange prop
  const [selectedRange, setSelectedRange] = useState<string>(() => {
    if (initialDateRange && initialDateRange.startDate && initialDateRange.endDate) {
      const startDate = initialDateRange.startDate.toLocaleDateString();
      const endDate = initialDateRange.endDate.toLocaleDateString();
      console.log('💼 DateRangeSelector: Initializing with Redux date range:', `${startDate} - ${endDate}`);
  ...
  // NEW: Update selectedRange when initialDateRange prop changes
  useEffect(() => {
    if (initialDateRange && initialDateRange.startDate && initialDateRange.endDate) {
      const startDate = initialDateRange.startDate.toLocaleDateString();
      const endDate = initialDateRange.endDate.toLocaleDateString();
      const newRange = `${startDate} - ${endDate}`;
      console.log('💼 DateRangeSelector: Updating display from Redux:', newRange);
      setSelectedRange(newRange);
    } else if (!initialDateRange) {
      console.log('💼 DateRangeSelector: Clearing display due to Redux clear');
  ...
  const handleClear = (event: React.MouseEvent) => {
    event.stopPropagation();
    console.log('💼 DateRangeSelector: Clearing date range');
    setSelectedRange('Select date range');
...
export function Financials() {
  ...
  // Redux selectors from masterFileSlice
  ...
  // NEW: Redux selectors for date range from dateRangeSlice
  const reduxStartDate = useAppSelector(selectFinancialsDashboardStartDate);
  const reduxEndDate = useAppSelector(selectFinancialsDashboardEndDate);
  const reduxDateRange = useAppSelector(selectFinancialsDashboardDateRange);
  const hasReduxDateRange = useAppSelector(selectHasFinancialsDashboardDateRange);
  ...
  // Company-location API state
  const [companyLocations, setCompanyLocations] = useState<CompanyLocation[]>([]);
  const [companiesLoading, setCompaniesLoading] = useState(false);
  const [companiesError, setCompaniesError] = useState<string>("");

  // NEW: Date range state - initialize from Redux
  const [selectedDateRange, setSelectedDateRange] = useState<any>(() => {
    if (hasReduxDateRange && reduxStartDate && reduxEndDate) {
      console.log('💼 Financials: Initializing from Redux date range:', { reduxStartDate, reduxEndDate });
      return {
        startDate: new Date(reduxStartDate),
        endDate: new Date(reduxEndDate),
        startDateStr: reduxStartDate,
        endDateStr: reduxEndDate
      };
    }
    return null;
  });
  ...
  // Refs for tracking changes with proper array handling
  ...
  // NEW: Sync Redux date range changes to local state
  useEffect(() => {
    if (hasReduxDateRange && reduxStartDate && reduxEndDate) {
      console.log('💼 Financials: Syncing Redux date range to local state:', { reduxStartDate, reduxEndDate });
    ...
    } else if (!hasReduxDateRange && selectedDateRange) {
      console.log('💼 Financials: Clearing local date range due to Redux clear');
      setSelectedDateRange(null);
    }
  }, [hasReduxDateRange, reduxStartDate, reduxEndDate]);
  ...
  // Proper change detection with array copies
  ...
  // Apply filters with proper array handling
  const applyFiltersAutomatically = React.useCallback(async () => {
    ...
    try {
      ...
      // Convert location IDs to names for API compatibility
      ...
      // Send ALL locations as array to backend
      const payload = {
      ...
      if (response.data) {
        ...
        // Update Redux filters with proper array copies
        ...
        dispatch(updateFinancialFilters({ 
          ...
          allLocationNames: [...locationNamesForApi]
        }));

        // Update refs with copies to avoid mutation
  ...
  // Auto-filtering effect with proper change detection
  useEffect(() => {
  ...
  // Handle company/location changes with proper array copies
  ...
  // NEW: Handle date range changes with Redux integration
  const handleDateRangeSelect = (dateRange: any) => {
    console.log('💼 Financials: Date range selected:', dateRange);
    ...
    if (dateRange) {
      ...
      console.log('💼 Financials: Date range saved to Redux:', {
      ...
      console.log('💼 Financials: Date range cleared from Redux');
    }
  };
        ...
        >
          ...
          {selectedCompanies.length > 0 && (
            <Box sx={{ 
              ...
              {/* NEW: Display Redux date range status */}
              {hasReduxDateRange && (
                <CompanyInfoChip
                  icon={<CalendarTodayIcon />}
                  label={`Date Range: ${reduxStartDate} to ${reduxEndDate}`}
                  variant="outlined"
                  size="small"
                  color="secondary"
                />
      ...
      >
        <CardContent sx={{ p: 4 }}>
          ...
          <Box sx={{ 
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              <Chip
                ...
                sx={{ fontWeight: 500 }}
              />
              {/* NEW: Redux date range status indicator */}
              {hasReduxDateRange && (
                <Chip
                  icon={<CalendarTodayIcon />}
                  label="Date Range Saved"
                  color="secondary"
                  size="small"
                  variant="outlined"
                  sx={{ fontWeight: 500 }}
          ...
          <Grid container spacing={4} sx={{ mb: 3 }} justifyContent="center">
            ...
            <Grid item xs={12} md={4}>
              <DateRangeSelectorComponent
                label="Date Range (Optional) - Redux Integrated"
                onDateRangeSelect={handleDateRangeSelect}
                onCancel={() => {
                  setSelectedDateRange(null);
                  dispatch(clearFinancialsDashboardDateRange());
                }}
                initialDateRange={selectedDateRange} // NEW: Pass selectedDateRange as initial value
              />
            </Grid>
          </Grid>
          ...
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1.5, mb: 3 }}>
            ...
            {/* NEW: Redux-integrated date range chip */}
          ...
          {!hasMinimumRequirements() && (
            <Alert severity="info" sx={{ mt: 2 }}>
              <Typography variant="body2">
                ...
                {hasReduxDateRange && <><br/><strong>Date range from Redux:</strong> {reduxStartDate} to {reduxEndDate}</>}
              </Typography>
            </Alert>
          )}
        </CardContent>
              ...
              {statsData.length > 0 ? (
                ...
                <Box sx={{ 
                  ...
                  >
                    ...
                    {hasReduxDateRange && <><br/>Date range loaded from Redux: {reduxStartDate} to {reduxEndDate}</>}
                  </Typography>
                </Box>
              )}
            </>
