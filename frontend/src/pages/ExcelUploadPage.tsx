  ...
  try {
    ...
    if (Array.isArray(data)) {
      ...
      data.forEach((dashboardData, index) => {
        ...
        console.log(
          `Categories after processing dashboard ${index + 1}:`,
          categories
        );
      });
    } else {
...
const ExcelUploadPage: React.FC = () => {
  ...
  const uploadFile = async (fileInfo: FileInfo, index: number) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        if (Array.isArray(response.data)) {
          ...
          response.data.forEach((dashboardData) => {
            ...
            if (dashboardName === "Sales Split") {
              console.log(
                "Sales Split dashboard detected in array - processing data"
              );
              console.log(
              ...
              console.log(
                "Product Mix dashboard detected in array - processing data"
              );
              ...
              console.log(
                "Financials dashboard detected in array - processing data"
              );
              ...
              console.log(
                "Sales Wide dashboard detected in array - processing data"
              );
        ...
        console.log(
          "✅ File upload completed successfully with categories:",
          extractedCategories
        );
        return true;
      } else {
  ...
  const uploadAllFiles = async () => {
    const pendingFiles = files.filter(
      (file) => file.status === "pending" && file.location.trim()
    );

    if (pendingFiles.length === 0) {
      setGeneralError(
        "No files ready for upload. Please add location to files."
      );
      return;
    }
      ...
      <Card sx={{ mb: 4 }}>
        <CardContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
            New files will use these settings by default. You can modify
            individual files after upload.
          </Typography>
        </CardContent>
      </Card>
