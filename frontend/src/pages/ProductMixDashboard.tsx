...
const prepareSalesDashboardData = (data: any): any => {
  ...
  console.log(
    "🎯 Preparing SalesDashboard data (PRESERVING TABLE1, USING TABLE11):",
    data
  );
  ...
  if (preparedData.table1) {
    console.log(
      "🔧 Preserving table1 formatting for KPI cards:",
      preparedData.table1
    );
  ...
  if (preparedData.table11) {
    console.log(
      "🎯 Processing table11 (Sales Categories from backend) before formatting:",
      preparedData.table11
    );

    // Transform table11 to match the expected table2 format for the dashboard
    preparedData.table2 = preparedData.table11.map(
    ...
    console.log(
      "✅ Table11 transformed to Table2 format for Sales Categories Performance:",
      preparedData.table2
    );
  }
  ...
  ["table3", "table4", "table5", "table6"].forEach((tableName) => {
    if (preparedData[tableName]) {
      preparedData[tableName] = preparedData[tableName].map((item: any) => ({
        ...
        Sales:
          typeof item.SalesRaw === "number"
            ? formatNumber(item.SalesRaw)
  ...
  console.log(
    "✅ SalesDashboard data prepared with TABLE1 PRESERVED and TABLE11->TABLE2 TRANSFORMATION:",
    preparedData
  );
  return preparedData;
};
...
const enhanceDataWithFormatting = (data: any): any => {
  ...
  console.log(
    "🔍 Raw data before formatting (looking for table11):",
    enhancedData
  );
  ...
  if (enhancedData.table1) {
    ...
    enhancedData.table1 = enhancedData.table1.map((item: any) => {
      ...
      const result = {
        ...
        net_sales: (() => {
          const rawValue = Array.isArray(item.net_sales)
            ? item.net_sales[0]
            : item.net_sales;
        ...
        orders: (() => {
          const rawValue = Array.isArray(item.orders)
            ? item.orders[0]
            : item.orders;
          const cleanValue = cleanAndParseValue(rawValue);
          const formatted = cleanValue > 0 ? formatNumber(cleanValue) : "0";
        ...
        qty_sold: (() => {
          const rawValue = Array.isArray(item.qty_sold)
            ? item.qty_sold[0]
            : item.qty_sold;
          const cleanValue = cleanAndParseValue(rawValue);
          const formatted = cleanValue > 0 ? formatNumber(cleanValue) : "0";
        ...
        total_quantity: (() => {
          const rawValue = Array.isArray(item.total_quantity)
        ...
        average_order_value: (() => {
          const rawValue = Array.isArray(item.average_order_value)
            ? item.average_order_value[0]
            : item.average_order_value;
          const cleanValue = cleanAndParseValue(rawValue);
          const formatted =
            cleanValue > 0 ? formatCurrency(cleanValue) : "$0.00";
          return formatted;
        })(),
        ...
        average_items_per_order: (() => {
          const rawValue = Array.isArray(item.average_items_per_order)
            ? item.average_items_per_order[0]
            : item.average_items_per_order;
          const cleanValue = cleanAndParseValue(rawValue);
          const formatted = cleanValue > 0 ? formatNumber(cleanValue) : "0";
          return formatted;
        })(),
        ...
        net_sales_change:
          item.net_sales_change?.[0] || item.net_sales_change || 0,
        ...
        average_order_value_change:
          item.average_order_value_change?.[0] ||
          item.average_order_value_change ||
          0,
        average_items_per_order_change:
          item.average_items_per_order_change?.[0] ||
          item.average_items_per_order_change ||
          0,
      };
    ...
    console.log(
      "✅ Table1 AFTER fix (should show cleaned values):",
      enhancedData.table1
    );
  }
  ...
  if (enhancedData.table11) {
    console.log(
      "🎯 Processing table11 (Sales Categories from backend) before formatting:",
      enhancedData.table11
    );

    enhancedData.table11 = enhancedData.table11.map(
      (item: any, index: number) => {
        ...
        const originalCategory =
          item["Sales Category"] || `Category ${index + 1}`;
        ...
        const formatted = {
          ...
          This_4_Weeks_Sales_Currency: formatCurrency(
            originalThisPeriodSales,
            false
          ),
          Last_4_Weeks_Sales_Formatted: formatNumber(originalLastPeriodSales),
          Last_4_Weeks_Sales_Currency: formatCurrency(
            originalLastPeriodSales,
            false
          ),
    ...
    console.log(
      "✅ Table11 after formatting (Sales Categories with enhanced display):",
      enhancedData.table11
    );
  }
  ...
  if (enhancedData.table2 && !enhancedData.table11) {
    console.log(
      "🎯 Processing legacy table2 (fallback when no table11):",
      enhancedData.table2
    );

    enhancedData.table2 = enhancedData.table2.map(
      (item: any, index: number) => {
        ...
        const formatted = {
          ...
          Sales:
            typeof originalSales === "number"
              ? formatNumber(originalSales)
              : originalSales,
          ...
          DisplaySales:
            typeof originalSales === "number"
              ? formatNumber(originalSales)
              : originalSales,
          DisplaySalesWithCurrency:
            typeof originalSales === "number"
              ? formatCurrency(originalSales, false)
              : originalSales,
          DisplayPercentage:
            typeof originalPercentage === "number"
              ? `${originalPercentage}%`
    ...
    console.log(
      "✅ Table2 after formatting (LEGACY - prefer table11):",
      enhancedData.table2
    );
  }
  ...
  if (enhancedData.table3) {
    enhancedData.table3 = enhancedData.table3.map((item: any) => ({
      ...
      DisplaySales:
        typeof item.Sales === "number"
          ? formatCurrency(item.Sales, false)
          : item.Sales,
    }));
  }
  ...
  if (enhancedData.table12) {
    console.log(
      "🎯 Processing table12 BEFORE T_Sales fix:",
      enhancedData.table12
    );
    ...
    enhancedData.table12 = enhancedData.table12.map((item: any) => {
      const formatted = {
        ...
        T_Sales:
          typeof item.T_Sales === "number"
            ? item.T_Sales
            : parseFloat(item.T_Sales) || 0,
        ...
        B_Sales:
          typeof item.B_Sales === "number"
            ? item.B_Sales
            : parseFloat(item.B_Sales) || 0,
        ...
        // Quantity fields
        T_Quantity:
          typeof item.T_Quantity === "number"
            ? item.T_Quantity
            : parseFloat(item.T_Quantity) || 0,
        T_Quantity_Formatted:
          typeof item.T_Quantity === "number"
            ? formatNumber(item.T_Quantity)
            : formatNumber(parseFloat(item.T_Quantity) || 0),

        B_Quantity:
          typeof item.B_Quantity === "number"
            ? item.B_Quantity
            : parseFloat(item.B_Quantity) || 0,
        B_Quantity_Formatted:
          typeof item.B_Quantity === "number"
        ...
        // Difference Sales
        Difference_Sales:
          typeof item.Difference_Sales === "number"
            ? item.Difference_Sales
            : parseFloat(item.Difference_Sales) || 0,
        Difference_Sales_Formatted:
          typeof item.Difference_Sales === "number"
        ...
        Cost:
          typeof item.Cost === "number" ? formatCurrency(item.Cost) : item.Cost,
        CostRaw: item.Cost,
        "Food Cost":
          typeof item["Food Cost"] === "number"
            ? formatCurrency(item["Food Cost"])
            : item["Food Cost"],
        ...
        "Food Cost %":
          typeof item["Food Cost %"] === "number"
            ? formatPercentage(item["Food Cost %"])
            : item["Food Cost %"],
      };
    ...
    console.log(
      "✅ Table12 AFTER T_Sales/B_Sales fix (should show proper currency):",
      enhancedData.table12
    );
  }
  ...
  enhancedData._formatting = {
    ...
    fixes_applied: {
      table1:
        "FIXED: Array extraction and pre-formatted value cleaning - now extracts single values from arrays for KPI cards",
      table11:
        "NEW: Sales Categories from backend - processes This_4_Weeks_Sales, Last_4_Weeks_Sales, Percent_Change",
      table12:
        "FIXED: T_Sales and B_Sales formatting - now properly handles currency formatting",
    },
    special_handling: {
      table11:
        "Primary Sales Categories Performance data from backend with enhanced display values",
      table2:
        "Legacy fallback for Sales Categories Performance (table11 takes precedence)",
      table12:
        "Menu Items Table with comprehensive formatting including T_Sales/B_Sales fixes",
    },
    priority: {
      sales_categories:
        "table11 > table2 (backend data takes precedence over legacy format)",
    },
  };

  console.log(
    "📊 Enhanced data with TABLE11 SUPPORT and ALL FIXES APPLIED:",
    enhancedData
  );
  return enhancedData;
};
...
export default function ProductMixDashboard() {
  ...
  const currentProductMixData = useMemo(() => {
    console.log(
      "🔍 Selecting ProductMix data for location:",
      currentProductMixLocation
    );
    console.log(
      "📁 Available files:",
      productMixFiles.map((f) => ({
        location: f.location,
        hasData: !!f.data,
    ...
    const locationFiles = productMixFiles.filter(
    ...
    console.log("✅ Selected data:", {
      ...
      tablesAvailable: selectedData
        ? Object.keys(selectedData).filter((k) => k.startsWith("table"))
        : [],
    });
  ...
  const isDataReady = useMemo(() => {
    ...
    const hasRequiredTables = !!(
      currentProductMixData?.table1 || currentProductMixData?.table11
    );
  ...
  useEffect(() => {
    if (
      ...
      reduxDispatch(
        setSelectedLocations([availableLocations[0].location_id.toString()])
      );
      console.log(
        "🎯 ProductMix: Auto-selected single location:",
  ...
  useEffect(() => {
    console.log("🔄 ProductMix Data Change Detected:", {
      ...
      availableTables: currentProductMixData
        ? Object.keys(currentProductMixData).filter((k) =>
            k.startsWith("table")
          )
        : [],
    });
    ...
    currentProductMixLocation,
    dataVersion,
    productMixFiles.length,
    dataUpdated,
  ]);

  // Check if filters have minimum requirements to trigger auto-filter
  const hasMinimumFilters = useCallback(
    (company: string, locations: string[]) => {
  ...
  const hasFiltersChanged = useCallback(
    (
      ...
      const locationsChanged =
        JSON.stringify(currentLocations.sort()) !==
        JSON.stringify(prev.locations.sort());
      ...
      const categoriesChanged =
        ...
        JSON.stringify(prev.categories.sort());
      const serversChanged =
        JSON.stringify(currentServers.sort()) !==
        JSON.stringify(prev.servers.sort());
      const menuItemsChanged =
        JSON.stringify(currentMenuItems.sort()) !==
        JSON.stringify(prev.menuItems.sort());
    ...
    if (availableLocations.length > 0) {
      return selectedLocations.map((id) => {
        const location = availableLocations.find(
          (loc) => loc.location_id.toString() === id
        );
  ...
  const handleLocationChange = (newValue: string[]) => {
    ...
    if (availableLocations.length > 0) {
      ...
      console.log(
        "📍 ProductMix: Converting location names to IDs for Redux:",
        {
          names: newValue,
          ids: locationIds,
        }
  ...
  const triggerAutoFilter = useCallback(async () => {
    ...
    if (!isInitialized) {
      console.log(
        "⏳ ProductMix: Skipping auto-filter - component not initialized"
      );
      return;
    }
    ...
    if (
      ...
      console.log(
        "⏭️ ProductMix: No filter changes detected - skipping auto-filter"
      );
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        location:
          effectiveSelectedLocations.length === 1
            ? effectiveSelectedLocations[0]
            : null, // Keep single location for backward compatibility
        startDate: formattedStartDate,
        endDate: formattedEndDate,
        servers: currentServers,
        categories: currentCategories,
      ...
      console.log(
        "🚀 Sending Product Mix auto-filter request with Redux state:",
        payload
      );
      ...
      console.log(
        "📥 Received Product Mix auto-filter response:",
        response.data
      );
      ...
      if (response.data) {
        ...
        const enhancedData = {
          ...
          appliedFilters: {
            ...
            location:
              effectiveSelectedLocations.length === 1
                ? effectiveSelectedLocations[0]
                : `${effectiveSelectedLocations.length} locations`, // Display string
            startDate: currentStartDate,
            endDate: currentEndDate,
            servers: currentServers,
            categories: currentCategories,
        ...
        console.log(
          "📊 Enhanced data with auto-filters, formatting, and Redux company:",
          enhancedData
        );
        ...
        dispatch(
          updateProductMixFilters({
            ...
            location:
              effectiveSelectedLocations.length === 1
                ? effectiveSelectedLocations[0]
                : effectiveSelectedLocations.join(","), // Join for display
            startDate: currentStartDate,
            endDate: currentEndDate,
            servers: currentServers.join(","),
            categories: currentCategories.join(","),
            ...
            selectedCategories: currentCategories,
            dateRangeType:
              currentStartDate && currentEndDate ? "Custom Date Range" : "",
          })
        );
        ...
        console.log(
          "✅ Product Mix auto-filters applied successfully for company:",
          selectedCompanyName,
          "locations:",
          effectiveSelectedLocations
        );
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          if (
            ...
            errorMessage =
              "Product Mix filter API endpoint not found. Is the server running?";
  ...
  useEffect(() => {
    ...
    console.log(
      "🔄 ProductMix: Filter change detected, triggering debounced auto-filter"
    );
    debouncedAutoFilter();
    ...
    <Grid item {...gridSizes}>
      <Box sx={{ position: "relative", width: "100%" }}>
        ...
        >
          Date Range{" "}
          {hasReduxDateRange && (
            <span style={{ color: "#1976d2" }}>• Redux</span>
          )}
        </InputLabel>
        <Button
          variant="outlined"
      ...
      {!selectedCompany && (
        <Alert severity="warning" sx={{ mb: 3 }}>
          <Typography variant="body2">
            Please select a company above to see available locations for
            filtering.
          </Typography>
        </Alert>
      )}
          ...
          <Grid container spacing={2} sx={{ width: "100%" }}>
            ...
            <Grid item {...gridSizes}>
              <MultiSelect
                ...
                icon={<PlaceIcon />}
                placeholder={
                  selectedCompany ? "Select locations" : "Select company first"
                }
              />
            </Grid>
                ...
                {isAutoFiltering ? (
                  ...
                  <Box sx={{ width: "100%" }}>
                    ...
                    <MenuAnalysisDashboard
                      key={`performance-${dataVersion}-${currentProductMixLocation}-${
                        salesDashboardData?.filterTimestamp || "original"
                      }`}
                    ...
                    <Box sx={{ my: 4, width: "100%" }}>
                      <Divider
                        sx={{ borderColor: "#e0e0e0", borderWidth: 1 }}
                      />
                    </Box>
                    ...
                    >
                      ...
                      <MenuItemsTable
                        key={`menu-items-${dataVersion}-${currentProductMixLocation}-${
                          formattedCurrentData?.filterTimestamp || "original"
                        }`}
              ...
              >
                {isAutoFiltering ? (
                  ...
                  <MenuAnalysisDashboardtwo
                    key={`menu-analysis-${dataVersion}-${currentProductMixLocation}-${
                      formattedCurrentData?.filterTimestamp || "original"
                    }`}
